<template>
  <div class="mx-auto flex max-w-7xl flex-col gap-8 text-gray-900">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <p class="text-sm uppercase tracking-[0.3em] text-blue-600">Employee Request Forms</p>
        <h1 class="mt-3 text-3xl font-semibold tracking-tight">Employee Request Form (ERF)</h1>
      </div>
      <button
        v-if="canCreateEmployeeRequestForm"
        type="button"
    ...
    <section class="max-w-full">
      <div class="rounded-xl border border-gray-200 bg-gray-50 p-5">
        <div class="mb-4 flex items-center justify-between">
          <h2 class="text-xl font-semibold">Employee Request Form List</h2>
          <button
            type="button"
            ...
            <table class="w-full text-left text-sm text-gray-600">
              <thead class="bg-gray-50 text-xs uppercase text-gray-500">
                <tr>
                  ...
                  <th class="px-4 py-3 font-medium min-w-[160px]">Approval Director HRD</th>
                  <th class="px-4 py-3 font-medium min-w-[160px]">Director HRD Date</th>
                  ...
                  <td class="px-4 py-3 text-right align-middle">
                    <RowActionsMenu
                      :actions="[
                        ...
                        {
                          label: 'Edit',
                          disabled: saving || !canUpdateEmployeeRequestForm,
...
import { exportEmployeeRequestFormPdf } from '@/presentation/utils/employeeRequestFormPdfTemplate'
import { useAuthViewModel } from '@/viewmodels/useAuthViewModel'
import { useEmployeeRequestFormViewModel } from '@/viewmodels/useEmployeeRequestFormViewModel'

const router = useRouter()
const { jobs, loading, error, saving, remove, refresh, close } = useEmployeeRequestFormViewModel()
...
const pageSize = 10
const page = ref(1)
const canCreateEmployeeRequestForm = computed(() => hasPermission('employee_request_form:create'))
const canUpdateEmployeeRequestForm = computed(() => hasPermission('employee_request_form:update'))
...
function goToCreate() {
  router.push('/employee-request-forms/create')
}

function goToDetail(id: string) {
  router.push(`/employee-request-forms/${id}`)
}

function goToEdit(id: string) {
  router.push(`/employee-request-forms/${id}/edit`)
}
...
function handleExportPdf(job: EmployeeRequestForm) {
  try {
    exportEmployeeRequestFormPdf(job)

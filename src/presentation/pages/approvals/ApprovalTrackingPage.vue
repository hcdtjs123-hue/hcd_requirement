  ...
  <div class="mx-auto flex max-w-6xl flex-col gap-8 text-gray-900">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <p class="mt-2 text-sm text-gray-600">
          Pantau status approval job request beserta history detail setiap tahap.
        </p>
      </div>
      <button
        type="button"
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      >
        ...
        <option v-for="opt in statusOptions" :key="opt" :value="opt">{{ statusLabel(opt as ApprovalChainStatus) }}</option>
      </select>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left text-sm text-gray-600">
          ...
          <tr>
            ...
            <th class="px-4 py-3 font-medium min-w-[170px]">Dikirim</th>
            <th class="px-4 py-3 font-medium min-w-[160px]">Status</th>
            <th class="px-4 py-3 font-medium min-w-[360px]">Tahapan Approval</th>
          </tr>
          </thead>
          <tbody class="divide-y divide-gray-200">
          <tr v-for="(chain, idx) in pagedChains" :key="chain.id" class="transition hover:bg-gray-50">
            ...
            <td class="px-4 py-3 align-top">
              <p class="font-semibold text-gray-900 text-base whitespace-nowrap">
                {{ chain.job_request?.main_position || "Tanpa Posisi" }}
              </p>
            </td>
            <td class="px-4 py-3 align-top">
              <p class="whitespace-nowrap">{{ chain.job_request?.designation || "-" }}</p>
            </td>
            <td class="px-4 py-3 align-top">
              ...
              <div class="space-y-2">
                ...
                >
                  ...
                  <div class="min-w-0 flex-1">
                    <p class="font-medium text-gray-900">{{ step.approver_name || step.approver_email }}</p>
                    <p v-if="step.notes" class="mt-0.5 italic text-gray-500 line-clamp-1">"{{ step.notes }}"</p>
                  </div>

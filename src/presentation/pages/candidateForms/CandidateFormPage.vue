    ...
    <div class="flex items-center gap-4">
      ...
      <div>
        ...
        <h1 class="mt-1 text-2xl font-semibold tracking-tight">
          {{ isEdit ? 'Edit Candidate Form' : 'New Candidate Form' }}
        </h1>
      </div>
    </div>

    <p
      v-if="candidateError || employeeRequestFormError"
      class="rounded-xl border border-red-300 bg-red-50 px-4 py-3 text-sm text-red-800"
    >
      {{ candidateError || employeeRequestFormError }}
    </p>
...
const { jobs, error: employeeRequestFormError, refresh: refreshJobs } = useEmployeeRequestFormViewModel()
...
function createEmptyForm(): CandidateFormInput {
  return {
...
function mapFamilyItem(
  item: CandidateForm['family_and_emergency'][number],
...
function mapEducationItem(item: CandidateForm['education'][number]): EducationHistoryInput {
  return {
    id: item.id,
    level: item.level ?? '',
...
function mapWorkHistoryItem(item: CandidateForm['work_history'][number]): WorkHistoryInput {
  return {
    id: item.id,
...
function mapPersonalStatement(
  personalStatement: CandidateForm['personal_statement'],
...
function loadData() {
  if (isEdit.value && id.value) {
    ...
    if (candidate) {
      ...
      appToast.error('Candidate form not found.')
      router.back()
    }
  }
}
...
function populateForm(candidateForm: CandidateForm) {
  Object.assign(form, {
    employee_request_form_id: candidateForm.employee_request_form_id ?? '',
    candidate_id: candidateForm.candidate_id ?? null,
    date_application: candidateForm.date_application ?? '',
    notice_period: candidateForm.notice_period ?? '',
    first_name: candidateForm.first_name ?? '',
    middle_name: candidateForm.middle_name ?? '',
    last_name: candidateForm.last_name ?? '',
    hire_location: candidateForm.hire_location ?? '',
    date_of_birth: candidateForm.date_of_birth ?? '',
    ...
    ethnic: candidateForm.ethnic ?? '',
    blood_type: candidateForm.blood_type ?? '',
    id_card_address: candidateForm.id_card_address ?? '',
    residential_address: candidateForm.residential_address ?? '',
    personal_email: candidateForm.personal_email ?? '',
    instagram: candidateForm.instagram ?? '',
    linkedin: candidateForm.linkedin ?? '',
    phone_number: candidateForm.phone_number ?? '',
    id_type: candidateForm.id_type ?? '',
    id_no: candidateForm.id_no ?? '',
    reference_name: candidateForm.reference_name ?? '',
    reference_no: candidateForm.reference_no ?? '',
    reference_relationship: candidateForm.reference_relationship ?? '',
    reference_position: candidateForm.reference_position ?? '',
    reference_transportation: candidateForm.reference_transportation ?? '',
    reference_ownership: candidateForm.reference_ownership ?? '',
    reference_residence: candidateForm.reference_residence ?? '',
    family_and_emergency: mapListOrDefault(
      candidateForm.family_and_emergency,
      mapFamilyItem,
      createFamilyItem,
    ),
    education: mapListOrDefault(candidateForm.education, mapEducationItem, createEducationItem),
    work_history: mapListOrDefault(
      candidateForm.work_history,
      mapWorkHistoryItem,
      createWorkHistoryItem,
    ),
    personal_statement: mapPersonalStatement(candidateForm.personal_statement),
  })
}
...
function buildPayload(): CandidateFormInput {
  return {
    ...form,
    gender: form.gender as CandidateFormInput['gender'],
    marital_status: form.marital_status as CandidateFormInput['marital_status'],
    blood_type: form.blood_type as CandidateFormInput['blood_type'],
...
async function handleSubmit() {
  try {
    ...
    router.push('/candidate-forms')
  } catch (err) {
    const message = err instanceof Error ? err.message : 'Failed to save the candidate form.'
